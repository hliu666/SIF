...
fitting = function(file, outFile){
  ...
  write.table(fit_Beck$predicted, file = outFile, sep =",", row.names =FALSE, col.names =FALSE, quote =FALSE)
}
...
for (Dataset in Datasets){
    for (Region in Regions){
      ...
      outPath = paste(root,"Fitting_",Dataset,"/",Region,"/SitePixel/Polygon/" ,sep ="")       
      fileLists = Sys.glob(paste(inPath, "*.csv", sep = ""))
      for (file in fileLists){
        outPath_Beck = paste(outPath, "/","DoubleLogBeck", sep ="")
